...
package org.eclipse.papyrus.requirements.sysml14.matrix.verifiedBy.config.cellmanager;
...
import org.eclipse.papyrus.requirements.sysml14.matrix.verifiedBy.config.Activator;
...
public class VerifiedByMatrixCellManager extends AbstractCellManager {
	...
	public boolean handles(Object columnElement, Object rowElement) {
		...
		if (rowElement instanceof ITreeItemAxis) {
			...
			if (!tableName.equals("VerifiedByMatrixSysML14")) {
				return false;
			}
		}
	...
	protected Object doGetValue(Object columnElement, Object rowElement, INattableModelManager tableManager) {
		...
		if (column instanceof Element) {
			...
			if (UMLUtil.getStereotypeApplication(colUMLElement, Requirement.class) != null) {
				...
				Object listOfObjectsThatVerifiesRequirement = colUMLElement.getValue(reqStereotype,
						I_SysMLStereotype.REQUIREMENT_VERIFIEDBY_ATT);
				if (listOfObjectsThatVerifiesRequirement instanceof EList<?>) {
					@SuppressWarnings("unchecked")
					EList<EObject> list = (EList<EObject>) listOfObjectsThatVerifiesRequirement;
					// check the list of elements that satisfy the requirement
					// to see if the element in the current row
					for (EObject eObjectThatVerifiesRequirement : list) {
						if (eObjectInRow.equals(eObjectThatVerifiesRequirement)) {
							return true;
						}
					}
				}
		...
		if (column instanceof Element) {
			...
			if (Boolean.FALSE.equals(newValue)) {
				...
				IElementType type = ElementTypeRegistry.getInstance()
						.getType(SysMLServiceTypeUtil.ORG_ECLIPSE_PAPYRUS_SYSML14_VERIFY_ABSTRACTION);
				if (row instanceof Element) {
