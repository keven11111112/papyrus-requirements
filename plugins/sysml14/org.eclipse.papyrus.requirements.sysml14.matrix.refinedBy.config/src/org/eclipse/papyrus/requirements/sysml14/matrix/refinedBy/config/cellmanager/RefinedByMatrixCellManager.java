...
package org.eclipse.papyrus.requirements.sysml14.matrix.refinedBy.config.cellmanager;
...
import org.eclipse.papyrus.requirements.sysml14.matrix.refinedBy.config.Activator;
...
public class RefinedByMatrixCellManager extends AbstractCellManager {
	...
	public boolean handles(Object columnElement, Object rowElement) {
		...
		if (rowElement instanceof ITreeItemAxis) {
			...
			if (!tableName.equals("RefinedByMatrixSysML14")) {
				return false;
			}
		}
	...
	protected Object doGetValue(Object columnElement, Object rowElement, INattableModelManager tableManager) {
		...
		if (column instanceof Element) {
			...
			if (UMLUtil.getStereotypeApplication(colUMLElement, Requirement.class) != null) {
				...
				Object listOfObjectsThatRefineTheRequirement = colUMLElement.getValue(reqStereotype, I_SysMLStereotype.REQUIREMENT_REFINEDBY_ATT);
				if (listOfObjectsThatRefineTheRequirement instanceof EList<?>) {
					@SuppressWarnings("unchecked")
					EList<EObject> list = (EList<EObject>) listOfObjectsThatRefineTheRequirement;
		...
		if (column instanceof Element) {
			...
			if (Boolean.FALSE.equals(newValue)) {
				if (UMLUtil.getStereotypeApplication(colUMLElement, Requirement.class) != null) {
					...
					I_SysMLStereotype.REFINE_STEREOTYPE);
				}
			} else if (Boolean.TRUE.equals(newValue)
					|| (newValue instanceof Abstraction && ((Abstraction) newValue).eContainer() == null)) {
				IElementType type = ElementTypeRegistry.getInstance().getType(SysMLServiceTypeUtil.ORG_ECLIPSE_PAPYRUS_SYSML14_REFINE_ABSTRACTION);
				if (row instanceof Element) {
