...
public class DerivedFromMatrixCellManager extends AbstractCellManager {
	...
	public boolean handles(Object columnElement, Object rowElement) {
		// Supports Requirements in column and Requirements in rows only
		...
		if (column instanceof Element) {
			...
			if (Boolean.FALSE.equals(newValue)) {
				if (UMLUtil.getStereotypeApplication(colUMLElement, Requirement.class) != null) {
					...
					Object listOfObjectsThatDerivedTheRequirement = colUMLElement.getValue(reqStereotype, I_SysMLStereotype.REQUIREMENT_DERIVEDFROM_ATT);

					if (listOfObjectsThatDerivedTheRequirement instanceof EList<?>) {
						@SuppressWarnings("unchecked")
						EList<EObject> list = (EList<EObject>) listOfObjectsThatDerivedTheRequirement;
						DirectedRelationship toDestroy = null;
						for (EObject eObjectThatDerivesTheRequirement : list) {
							if (toDestroy == null) {
								EObject baseClass = UMLUtil.getBaseElement(eObjectThatDerivesTheRequirement);
				...
				CreateRelationshipRequest request = new CreateRelationshipRequest(domain, container, (EObject) column,
						rowUMLElement, ElementTypeRegistry.getInstance().getType(SysMLServiceTypeUtil.ORG_ECLIPSE_PAPYRUS_SYSML14_DERIVEREQT_ABSTRACTION));
